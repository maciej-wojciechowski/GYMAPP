...
function editTrainingPopup(e) {
    ...
    const exercises = Object.keys(editedTraining).filter(entry => entry !== 'date');
    exercises.forEach(exer => {
        ...
        for (let set in editedTraining[exer]) {
            entry.appendChild(createSet(editedTraining[exer][set][0], editedTraining[exer][set][1]))
        }
    ...
    if (!exercises.includes("Deadlift")){
        select.innerHTML += '<option id="deadlift-sel" value="Deadlift">Deadlift</option>';
    }
...
function saveTraining(e) {
    ...
    if ( conf === true) {
        ...
        return db.collection('trainings').doc(currentUser.uid).update({
            [id]: trainingObj
        }).then(getTrainings()).then(closeEdit())
