...
function addDelete(e) {
    ...
    if (target.classList.contains('add-set-btn')) {
        entry.insertBefore(createSet(), entry.querySelector('.add-set-btn'));
    }
...
function save(e) {
    ...
    db.collection('trainings').doc(user.uid).get().then( doc => {
        if (doc.exists) {
            ...
            db.collection('trainings').doc(user.uid).update({[id]: trainingObj})
        } else {
            trainings[id]= trainingObj
            db.collection('trainings').doc(user.uid).set(trainings, { merge: true}).then(
                alert("Your first training added!")
                        ).catch(err => {
    ...
    // const id = uuidv4();
    // const trainingObj = { date };
    // const liEntries s= document.querySelectorAll('.entry li')
    // liEntries.forEach(entry => {
    //     const exName = entry.querySelector('.exercise-span').innerText;
    //     const sets = entry.parentElement.querySelectorAll('.set')
    //     const exerObj = {};
    ...
    // trainings = {};
    // sortableTrainings.forEach(entry => {
    //     trainings[entry[0]] = entry [1];
    // });
    // auth.onAuthStateChanged(user => {
    //     if (user) {
    //         db.collection('trainings').doc(user.uid).set(trainings, { merge: true}).then(
    ...
    //     }
    //     else {
    //         alert('User is not signed in to add TRAININGS');
    //         location = index.html
    //     }
    // });
